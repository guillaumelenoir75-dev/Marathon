...
async function _sendFcReposToAll(db, tag, title, body) {
  const today = new Date().toISOString().slice(0,10);
  // Admin
  try {
    if (await getUserPref(db, ADMIN_STATE, 'notif_fc_repos')) {
      const snap = await db.ref(`${ADMIN_STATE}/fc_repos_${today}`).once('value');
      if (!snap.val()) await sendPush(VAPID_PUBLIC_KEY.value(), VAPID_PRIVATE_KEY.value(), title, body, tag, '/');
    }
  } catch(e) { console.error(`${tag} admin:`, e.message); }
  ...
  try {
    ...
    for (const [uid, sub] of Object.entries(subs)) {
      ...
      if (!await getUserPref(db, `users/${uid}/state`, 'notif_fc_repos')) continue;
      const fcSnap = await db.ref(`users/${uid}/state/fc_repos_${today}`).once('value');
      if (fcSnap.val()) continue;
      await sendPushToUser(db, uid, sub, VAPID_PUBLIC_KEY.value(), VAPID_PRIVATE_KEY.value(), title, body, tag);
    }
  } catch(e) { console.error(`${tag} users:`, e.message); }
}
...
exports.fcReposReminderWeekday = onSchedule(
  ...
  async()=>{ const db=admin.database(); await _sendFcReposToAll(db,'fc-repos','Rappel ☀️','Rentre ta FC repos avant de te lever ❤️'); }
);
...
exports.fcReposReminder14hWeekday = onSchedule(
  ...
  async()=>{ const db=admin.database(); await _sendFcReposToAll(db,'fc-repos-14h','Rappel',"Tu n'as pas encore rentré ta FC repos aujourd'hui ❤️"); }
);
...
exports.sessionReminder = onSchedule(
  ...
  async()=>{
    ...
    const adminStateSnap=await db.ref(ADMIN_STATE).once('value');
    const adminStateForCw=adminStateSnap.val()||{};
    const cw=getWeekFromDB(adminStateForCw.plan_start_date);
    const now=new Date();
...
exports.briefAfterFcRepos = onSchedule(
  ...
  async()=>{
    try{
      ...
      await db.ref(`${ADMIN_STATE}/_brief_trigger`).remove();
      const cw=getWeekFromDB(state.plan_start_date);
  ...
  async()=>{
    try{
      ...
      for(const [uid,sub] of Object.entries(allSubs)){
        ...
        const manquees=[];
        let extraI=0;
        while(uState[`extra_w${ucw}_s${extraI}`]){
          const done=!!uState[`extra_w${ucw}_s${extraI}_done`];
          if(!done){
            let es;try{es=JSON.parse(uState[`extra_w${ucw}_s${extraI}`]);}catch(e){extraI++;continue;}
            if(es.sched_day===dayOfWeek){
              const titre=es.d?es.d.split('|')[0]:(es.type||'').toUpperCase();
              manquees.push(`🏃 ${titre} ${es.km||''}km`);
            }
  ...
  async()=>{
    try{
      ...
      for(const [uid,sub] of Object.entries(allSubs)){
        ...
        const uState=(await db.ref(`users/${uid}/state`).once('value')).val()||{};
        const ucw=getWeekFromDB(uState.plan_start_date);
        const uCongratsKey=`_congrats_sent_w${ucw}`;
        if(uState[uCongratsKey])continue;
        let totalSessions=0,doneSessions=0;
        let si=0;
        while(uState[`extra_w${ucw}_s${si}`]!==undefined&&uState[`extra_w${ucw}_s${si}`]!==null){
          let es;try{es=JSON.parse(uState[`extra_w${ucw}_s${si}`]);}catch(e){si++;continue;}
          if(es&&es.type!=='rest'){totalSessions++;if(!!uState[`extra_w${ucw}_s${si}_done`])doneSessions++;}
          si++;
          if(si>20)break;
        }
        if(totalSessions===0||doneSessions<totalSessions)continue;
        const lastValid=uState[`_last_validation_w${ucw}`]||0;
        const ageMin=(now-lastValid)/1000/60;
        if(ageMin>20)continue;
        const body=`S${ucw} complète ! ${doneSessions} séance${doneSessions>1?'s':''} validée${doneSessions>1?'s':''}. Belle semaine, continue ! 💪`;
        await sendPushToUser(db,uid,sub,VAPID_PUBLIC_KEY.value(),VAPID_PRIVATE_KEY.value(),`🎉 Semaine S${ucw} complète !`,body,'week-complete');
  ...
  async()=>{
    try{
      ...
      if(await getUserPref(db,ADMIN_STATE,'notif_planif')){
        const adminSt=(await db.ref(ADMIN_STATE).once('value')).val()||{};
        const cw=getWeekFromDB(adminSt.plan_start_date);
      ...
      for(const [uid,sub] of Object.entries(allSubs)){
        ...
        const uSt=(await db.ref(`users/${uid}/state`).once('value')).val()||{};
        const ucw=getWeekFromDB(uSt.plan_start_date);
        const ucwNext=Math.min(ucw+1,32);
        const body=`Prends 2 min pour planifier tes séances de la semaine S${ucwNext} à venir 🏃`;
        await sendPushToUser(db,uid,sub,VAPID_PUBLIC_KEY.value(),VAPID_PRIVATE_KEY.value(),'📅 Plan S'+ucwNext,body,'planif-reminder');
      }
    ...
    try{
      ...
      for(const [uid,sub] of Object.entries(allSubs)){
        ...
        const body=totalSessions>0?
          `S${ucw} : ${doneSessions}/${totalSessions} séance${totalSessions>1?'s':''} validée${doneSessions>1?'s':''}. S${ucwNext} arrive, prépare-toi ! 🏃`:
          `S${ucw} terminée ! Ouvre l'app pour voir le programme S${ucwNext} 👇`;
        await sendPushToUser(db,uid,sub,VAPID_PUBLIC_KEY.value(),VAPID_PRIVATE_KEY.value(),`📊 Bilan S${ucw}`,body,'weekly-debrief');
      }
